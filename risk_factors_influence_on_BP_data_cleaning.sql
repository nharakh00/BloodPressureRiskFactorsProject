...
FROM
    smoking_status
WHERE
    patient_id IS NULL OR is_smoking IS NULL
...
WHERE
    BPMeds IS NULL
        OR prevalentStroke IS NULL
        OR PrevalentHyp IS NULL
        OR sysBP IS NULL
        OR diaBP IS NULL
        OR heartRate IS NULL
        OR TenYearCHD IS NULL;
...
FROM
    other_risk_factors
WHERE
    diabetes IS NULL OR totChol IS NULL
        OR BMI IS NULL
        OR glucose IS NULL;
...
SELECT 
    COUNT(DISTINCT CASE
            WHEN age BETWEEN 32 AND 37 THEN id
            ELSE NULL
        END) AS '32-37',
    COUNT(DISTINCT CASE
            WHEN age BETWEEN 38 AND 43 THEN id
            ELSE NULL
        END) AS '38-43',
    COUNT(DISTINCT CASE
            WHEN age BETWEEN 44 AND 49 THEN id
            ELSE NULL
        END) AS '44-49',
    COUNT(DISTINCT CASE
            WHEN age BETWEEN 50 AND 55 THEN id
            ELSE NULL
        END) AS '50-55',
    COUNT(DISTINCT CASE
            WHEN age > 55 THEN id
...
SELECT 
    COUNT(DISTINCT CASE
            WHEN cigsPerDay = 0 THEN patient_id
            ELSE NULL
        END) AS 'non user',
    COUNT(DISTINCT CASE
            WHEN cigsPerDay > 0 AND cigsPerDay < 20 THEN patient_id
            ELSE NULL
        END) AS 'avg user',
    COUNT(DISTINCT CASE
            WHEN cigsPerDay >= 20 THEN patient_id
            ELSE NULL
        END) AS 'heavy user'
FROM
    smoking_status; 
...
-- dropping prevalent_hyp and blood sys blood pressure range will indicate hypertension or high blood pressure 
-- Not going to use heart_rate
...
SELECT 
    COUNT(DISTINCT CASE
            WHEN sys_bp < 120 THEN patient_id
            ELSE NULL
        END) AS 'normal',
    COUNT(DISTINCT CASE
            WHEN sys_bp BETWEEN 121 AND 139 THEN patient_id
            ELSE NULL
        END) AS 'at risk',
    COUNT(DISTINCT CASE
            WHEN sys_bp >= 140 THEN patient_id
            ELSE NULL
        END) AS 'high blood pressure'
FROM
    cardiovascular_info; 
...
SELECT 
    COUNT(DISTINCT CASE
            WHEN totChol < 200 THEN patient_id
            ELSE NULL
        END) AS 'desirable',
    COUNT(DISTINCT CASE
            WHEN totChol BETWEEN 200 AND 239 THEN patient_id
            ELSE NULL
        END) AS 'borderline high',
    COUNT(DISTINCT CASE
            WHEN totChol >= 240 THEN patient_id
            ELSE NULL
        END) AS 'high'
FROM
    other_risk_factors; 
...
SELECT 
    COUNT(DISTINCT CASE
            WHEN BMI < 18.5 THEN patient_id
            ELSE NULL
        END) AS 'underweight',
    COUNT(DISTINCT CASE
            WHEN BMI BETWEEN 18.5 AND 24.9 THEN patient_id
            ELSE NULL
        END) AS 'healthy range',
    COUNT(DISTINCT CASE
            WHEN BMI BETWEEN 24.91 AND 29.9 THEN patient_id
            ELSE NULL
        END) AS 'overweight',
    COUNT(DISTINCT CASE
            WHEN BMI BETWEEN 29.91 AND 39.9 THEN patient_id
            ELSE NULL
        END) AS 'obese',
    COUNT(DISTINCT CASE
            WHEN BMI > 39.91 THEN patient_id
            ELSE NULL
        END) AS 'severe obesity'
FROM
    other_risk_factors; 
